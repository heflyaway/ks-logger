...
function createLogger({ serviceName, customLevels = [], daysToKeep = 1, logRolling = 'day', debug = false, defaultLog = {}, singleLogFile = true, useServiceName = false, logDir, }) {
    const levels = {};
...
function getLogFilename({ serviceName, appenderName, singleLogFile, useServiceName, logDir, }) {
    const logPath = logDir || path_1.join(process.cwd(), 'logs');
    let filename = 'common';
    if (useServiceName) {
